...
void trendline_test() {
    ...
    string symbol = config.get("symbol");
    TradeReader trade_reader(symbol);
    TrendLineUp2Points * trendline = new TrendLineUp2Points(
        config.get_double("trendline_zigzag"),
        config.get_double("trendline_zigzag_correction_multiplier") * config.get_double("trendline_zigzag"),
    ...
    pubsub.subscribe(trendline->zigzag->topic_appends, [&zigzag_file, &trendline](void* data) {
    ...
    pubsub.subscribe(trendline->publish_topic, [&trend_line_file](void* data) {
        char* buffer = static_cast<char*>(data);
        // cout << "Received up trend line data." << endl;
        trend_line_file.write(buffer, 48); // Write 48 bytes of the trend line data
    });
